...
<form method="post">
    <div id="editor">
        <div>
            ...
            <textarea id="source" name="source"><?php echo $_POST['source'] ?? file_get_contents('index.less') ?></textarea>
            <div class="tools">
    ...
    <div id="extra">
        <div>
            ...
            <div>
                <input type="checkbox" id="remove_ie_hacks" name="ie_compatible" <?php echo ($process_data['settings']['ie_compatible'] ?? false) ? 'checked' : ''; ?> /><label
                    for="ie_compatible" title="Try to generate a CSS compatible with old IE versions">IE compatible</label>
            </div>
        </div>
        <div>
            <h3>Prefix</h3>
            <div>
                <div>
                    <input type="checkbox" id="prefix-webkit" name="prefix[webkit]" <?php echo $process_data['settings']['prefixes'] == 'all' || $process_data['settings']['prefixes']['webkit'] ? 'checked' : ''; ?> /><label
                        for="prefix-webkit" title="Add prefix for webkit-based browser such as Chrome or Safari">Webkit</label>
                </div>
                <div>
                    <input type="checkbox" id="prefix-mozilla" name="prefix[mozilla]" <?php echo $process_data['settings']['prefixes'] == 'all' || $process_data['settings']['prefixes']['mozilla'] ? 'checked' : ''; ?> /><label
                        for="prefix-mozilla" title="Add prefix for Mozilla Firefox">Firefox</label>
                </div>
                <div>
                    <input type="checkbox" id="prefix-opera" name="prefix[opera]" <?php echo $process_data['settings']['prefixes'] == 'all' || $process_data['settings']['prefixes']['opera'] ? 'checked' : ''; ?> /><label
                        for="prefix-opera" title="Add prefix for Opera Browser">Opera</label>
                </div>
                <div>
                    <input type="checkbox" id="prefix-microsoft"
                           name="prefix[microsoft]" <?php echo $process_data['settings']['prefixes'] == 'all' || $process_data['settings']['prefixes']['microsoft'] ? 'checked' : ''; ?> /><label for="prefix-microsoft"
                                                                                                                                              title="Add prefix for Internet Explorer">Internet
                        Explorer</label>
                </div>
            </div>
        </div>
        <?php if (isset($process_data['css'])): ?>
            <div>
                <h3>Statistics</h3>
                <ul>
                    <li>Original size: <?php echo readable_size(strlen($process_data['source'])) ?> (<?php echo strlen(gzencode($process_data['source'], 9)) ?> bytes
                        gzipped)
                    </li>
                    ...
                    <li>Difference: <strong><?php printf(
                                '%s (%+g%%)',
                                readable_size(strlen($process_data['css']) - strlen($process_data['source']), true),
                                round(strlen($process_data['css']) / strlen($process_data['source']) * 100, 2)
                            ) ?></strong></li>
                </ul>
                <ul>
...
function do_optimization(): array
{
    ...
    if (empty($_POST)) {
        ...
        foreach (['webkit', 'mozilla', 'opera', 'microsoft'] as $type) {
            $data['settings']['prefixes'][$type] = isset($_POST['prefixes'][$type]);
        }
    }
...
function optimize($css, $settings, &$errors = null): string
{
...
function readable_time($time): string
{
    if ($time > 60) {
        $min = floor($time / 60);
        $sec = round($time) % 60;
...
function readable_size($bytes, $sign = false, $precission = 2): string
{
    if ($bytes > 1000000000) {
