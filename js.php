...
/**
 * Rutinas de compresión y procesado de código Javascript
 *
 * @package Tinyfier
 */
class JS {
    ...
    /**
     * Procesa el código Javascript indicado en $source
     *
     * Opciones disponibles:
     *
     * 'pretty': valor booleano que indica si el javascript se formateará de forma legible para humanos (aumenta el tamaño final)
     *
     * 'gclosure': valor booleano que indica si se usará google closure para comprimir el código fuente
     *
     * @param string $source
     * @param array $settings
     * @return string
     */
    public static function Process($source, $settings = null) {
        ...
        //Comprimir usando Google Closure (si es posible)
        if ($settings['gclosure'] && strlen($source) > 750) {
            $compiled = self::CompileGoogleClosure($source, $settings['pretty']);
            if ($compiled !== false)
                return $compiled;
        }

        //En caso contrario, usar JSMinPlus
        if ($settings['pretty']) {
            return $source;
        } else {
    ...
    /**
     * Compila código Javascript usando la API del compilador Google Closure
     * @param string $source Código fuente javascript
     * @param string $pretty Valor que indica si se identará el código de entrada para una mejor legibilidad
     * @return mixed Resultado de la operación, false si se ha producido un error
     */
    public static function CompileGoogleClosure($source, $pretty=false) {
        if (!function_exists('curl_exec'))
            return false;

        $ch = curl_init('http://closure-compiler.appspot.com/compile');

        $postData = 'output_info=compiled_code&output_format=text';
        if ($pretty) {
            $postData .= '&formatting=pretty_print&compilation_level=WHITESPACE_ONLY';
        } else {
            $postData .= '&compilation_level=SIMPLE_OPTIMIZATIONS';
        }
        $postData .= '&js_code=' . urlencode($source);
